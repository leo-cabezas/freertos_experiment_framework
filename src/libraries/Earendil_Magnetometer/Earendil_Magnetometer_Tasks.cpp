...
namespace Earendil_Magnetometer {
    ...
    void vMagnetometer_UpdateHeading(void* pvParameters){
        ...
        const TickType_t xFrequency = pdMS_TO_TICKS(PERIOD_VMAGNETOMETER_UPDATEHEADING_MS);
        ...
        while (Earendil_Data->Experimental_Data.experiment_ongoing){
            ...
            Earendil_Data->Magnetometer_Data.heading_deg = getHeading(); // Task execution
            ...
            if (Earendil_Data->Experimental_Data.collect_magnetometer_logs){
                auto& log_array     = Earendil_Data->Experimental_Data.magnetometer_logs;
                auto& log_array_idx = Earendil_Data->Experimental_Data.magnetometer_logs_idx;
